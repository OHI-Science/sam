...
Let's look at the list of species that are represented in the FIS model as mean catch: (the [fis_meancatch](https://github.com/OHI-Science/sam/blob/master/eez/layers/fis_meancatch.csv) layer). This list is in some ways a historic list because it includes species that may not be caught any more (i.e. was fished in the past and stopped for some reason). This is the average of each species' catch through time, so if it was a big catch in the past but now is 0, it will still average (it keeps all species in the dataset).
...
There are `r length(unique(stock_sam$taxon_scientific_name))` unique species
