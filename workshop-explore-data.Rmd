...
The Species goal has `r nrow(rgn_spp_gl)` unique species. This is probably more than we can go through today. But, here is a table to look through if we have time:
...
These are the species included in the ICO goal: 

```{r ico list, echo=FALSE}
## filter and join species name
...
There is a more reasonable number of species listed for Samoa at the moment: `r nrow(ico_spp_code)`. This is something that we could explore and we could potentially recalculate together. 
